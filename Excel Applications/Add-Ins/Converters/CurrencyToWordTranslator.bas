...
'=====================================================================\
'=====================================================================|\
'Add-in Name: CurrencyTranslator                                      ||\
'Version: 1.0                                                         |||\
...
Const strNumberLimitMsg As String = "Please insert a Number between (-)99,99,99,99,999.99 to (+)99,99,99,99,999.99"
...
Sub CallForHelp(ByVal Control As IRibbonControl)
    uHelp.Show
End Sub

Public Function F_CurToWrds_Hindi(ByVal MyNumber As Variant, _
    ...
    'iscurrency?
    pIsCurrencyString = CurrencyForm
    
    'Call Engine
    F_CurToWrds_Hindi = Translator(MyNumber, opt_CurrencyStringPosition)

End Function

Public Function F_CurToWrds_Eng_Ind(ByVal MyNumber As Variant, _
                                        Optional opt_CurrencyStringPosition As Variant = DefaultVal) As String
    ...
    'Call Engine
    F_CurToWrds_Eng_Ind = Translator(MyNumber, opt_CurrencyStringPosition)

End Function

Public Function F_CurToWrds_Guj(ByVal MyNumber As Variant, _
                                Optional opt_CurrencyStringPosition As Variant = DefaultVal) As String
    ...
    'Call Engine
    F_CurToWrds_Guj = Translator(MyNumber, opt_CurrencyStringPosition)

End Function

Public Function F_NumToWrds_Hindi(ByVal MyNumber As Variant) As String
    ...
    'Call Engine
    F_NumToWrds_Hindi = Translator(MyNumber)

End Function

Public Function F_NumToWrds_Eng_Ind(ByVal MyNumber As Variant) As String
    ...
    'Call Engine
    F_NumToWrds_Eng_Ind = Translator(MyNumber)

End Function

Public Function F_NumToWrds_Guj(ByVal MyNumber As Variant) As String
    ...
    'Call Engine
    F_NumToWrds_Guj = Translator(MyNumber)

End Function

Public Function F_TranslateDigitsTo_Hindi(ByVal MyNumber As Variant) As String
    
    'Storing Value in Property
    pInputCurrency = HindiChar
    
    'Call Engine
    F_TranslateDigitsTo_Hindi = Traslator_Digit(MyNumber)

End Function

Public Function F_TranslateDigitsTo_Guj(ByVal MyNumber As Variant) As String
    
    'Storing Value in Property
    pInputCurrency = GujaratiChar
    
    'Call Engine
    F_TranslateDigitsTo_Guj = Traslator_Digit(MyNumber)

End Function

Private Function Traslator_Digit(ByVal MyNumber As Variant) As Variant
    Dim myNumLen As Byte
    Dim newString As String
    Dim myChar As String
    Dim I As Byte
    Const COMP_STRING As String = "0123456789,."
    'Validation
    If VBA.IsNumeric(MyNumber) = False Or MyNumber = vbNullString Then
        Traslator_Digit = strInvalidMsg
        Exit Function
    End If
    'Initializing the runtime parameters
    myNumLen = VBA.Len(MyNumber)
    newString = vbNullString
    'Main logic to covert string by looking up a private function
    If myNumLen > 0 Then
        For I = 1 To myNumLen Step 1
            myChar = VBA.Mid(MyNumber, I, 1)
            If VBA.InStrB(COMP_STRING, myChar) > 0 Then
                newString = newString & LookupDString(myChar)
            End If
        Next I
        Traslator_Digit = newString
    Else
        Traslator_Digit = strInvalidMsg
    End If
End Function
...
Private Function Translator(ByVal MyNumber As Variant, _
    ...
    DecimalPlace = InStr(MyNumber, Application.DecimalSeparator)
    ...
    With Application.WorksheetFunction
        myArab = .RoundDown((MyNumber / 1000000000), 0)
        myCrore = .RoundDown(((MyNumber - (myArab * 1000000000)) / 10000000), 0)
        myLac = .RoundDown(((MyNumber - (myArab * 1000000000) - (myCrore * 10000000)) / 100000), 0)
        myThousand = .RoundDown(((MyNumber - (myArab * 1000000000) - (myCrore * 10000000) - (myLac * 100000)) / 1000), 0)
        MyHundred = .RoundDown(((MyNumber - (myArab * 1000000000) - (myCrore * 10000000) - (myLac * 100000) - (myThousand * 1000)) / 100), 0)
        myTen = .RoundDown(((MyNumber - (myArab * 1000000000) - (myCrore * 10000000) - (myLac * 100000) - (myThousand * 1000) - (MyHundred * 100))), 0)
    End With
...
Private Function LookupDString(ByVal myChar As Variant) As String
    Select Case myChar
        Case 0
            If pInputCurrency = GujaratiChar Then LookupDString = VBA.ChrW(2790)
            If pInputCurrency = HindiChar Then LookupDString = VBA.ChrW(2406)
        Case 1
            If pInputCurrency = GujaratiChar Then LookupDString = VBA.ChrW(2791)
            If pInputCurrency = HindiChar Then LookupDString = VBA.ChrW(2407)
        Case 2
            If pInputCurrency = GujaratiChar Then LookupDString = VBA.ChrW(2792)
            If pInputCurrency = HindiChar Then LookupDString = VBA.ChrW(2408)
        Case 3
            If pInputCurrency = GujaratiChar Then LookupDString = VBA.ChrW(2793)
            If pInputCurrency = HindiChar Then LookupDString = VBA.ChrW(2409)
        Case 4
            If pInputCurrency = GujaratiChar Then LookupDString = VBA.ChrW(2794)
            If pInputCurrency = HindiChar Then LookupDString = VBA.ChrW(2410)
        Case 5
            If pInputCurrency = GujaratiChar Then LookupDString = VBA.ChrW(2795)
            If pInputCurrency = HindiChar Then LookupDString = VBA.ChrW(2411)
        Case 6
            If pInputCurrency = GujaratiChar Then LookupDString = VBA.ChrW(2796)
            If pInputCurrency = HindiChar Then LookupDString = VBA.ChrW(2412)
        Case 7
            If pInputCurrency = GujaratiChar Then LookupDString = VBA.ChrW(2797)
            If pInputCurrency = HindiChar Then LookupDString = VBA.ChrW(2413)
        Case 8
            If pInputCurrency = GujaratiChar Then LookupDString = VBA.ChrW(2798)
            If pInputCurrency = HindiChar Then LookupDString = VBA.ChrW(2414)
        Case 9
            If pInputCurrency = GujaratiChar Then LookupDString = VBA.ChrW(2799)
            If pInputCurrency = HindiChar Then LookupDString = VBA.ChrW(2415)
        Case "."
            LookupDString = VBA.ChrW(46)
        Case ","
            LookupDString = VBA.ChrW(44)
    End Select
End Function
